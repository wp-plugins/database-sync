...
1. Install plugin on each server.
2. Go to Tools > Database Sync on the main server and copy the token.
3. On the other server(s), go to Tools > Database Sync and paste the token.
4. The main server URL now appears in the list of linked sites.
5. Click on the sync button then select either pull or push.

= Backups =

The plugin will attempt to make backups of the database before overwriting. These are stored as gzipped SQL
files in wp-content/plugins/database-sync/backups/dbYYYYMMDD.HHMMSS.sql.gz
